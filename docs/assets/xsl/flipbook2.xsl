<?xml version="1.0" encoding="UTF-8"?>
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:tei="http://www.tei-c.org/ns/1.0"
    xmlns:html="http://www.w3.org/1999/xhtml" exclude-result-prefixes="xs tei html" version="2.0">
    <xsl:output method="html"/>
    
    <!-- transform the root element (TEI) into an HTML template -->
    <xsl:template match="tei:TEI">
        <xsl:text disable-output-escaping='yes'>&lt;!DOCTYPE html&gt;</xsl:text><xsl:text>&#xa;</xsl:text>
        <html lang="en" xml:lang="en">
            <head>
                <title>
                    <!-- add the title from the metadata. This is what will be shown
                         on your browsers tab-->
                    Det elektriska ljuset: Hem
                </title>
                <!-- load bootstrap css (requires internet!) so you can use their pre-defined css classes to style your html -->
                <link rel="stylesheet"
                    href="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css"
                    integrity="sha384-ggOyR0iXCbMQv3Xipma34MD+dH/1fQ784/j6cY/iJTQUOhcWr7x9JvoRxT2MZw1T"
                    crossorigin="anonymous"/>
                <!-- first: the main site style (menu, background, etc) -->
                <link rel="stylesheet" type="text/css" href="assets/css/style.css"/>
                <!-- second: the specific viewer style -->
                <link rel="stylesheet" type="text/css" href="assets/css/flipbook.css"/>
            </head>
            <body>
                <header>
                    <h1>
                        <xsl:apply-templates select="//tei:titleStmt/tei:title"/>
                    </h1>
                </header>
                <nav id="sitenav">
                    <a href="index.html">Hem</a> |
                    <a href="transkribering.html">Transkribering</a> |
                    <a href="galleri_zoom.html">Galleri</a> | 
                    <a href="flipbook.html">Bl&#228;dderl&#228;ge</a>
                </nav>
                
                <main class="gallery-container">
                    <div id="gallery-container">
                        <!-- Flipbook Image Container -->
                        <div id="book">
                            <xsl:for-each select="//tei:facsimile/tei:surface/tei:figure/tei:graphic">
                                <div class="page">
                                    <!-- Dynamically assign 'active' class to the first image frame -->
                                    <xsl:if test="position() = 1">
                                        <xsl:attribute name="class">page active</xsl:attribute>
                                    </xsl:if>
                                    
                                    <!-- Map TEI attributes safely to clean HTML img attributes -->
                                    <img>
                                        <xsl:attribute name="src">
                                            <xsl:value-of select="@url"/>
                                        </xsl:attribute>
                                        <xsl:attribute name="alt">
                                            <xsl:value-of select="../tei:label"/>
                                        </xsl:attribute>
                                    </img>
                                
                                <!-- New Caption Overlay Section -->
                                <div class="image-caption">
                                    <h4><xsl:value-of select="../tei:label"/></h4>
                                   <!-- <h4><xsl:value-of select="../tei:label"/></h4> -->
                                    <!-- <p><xsl:value-of select="../tei:figDesc"/></p> -->
                                </div>
                                </div>
                            </xsl:for-each>
                        </div>
                           
                        <!-- Navigation Controls -->
                        <div class="controls">
                            <button id="prevBtn" onclick="changePage(-1)" disabled="disabled">Bak</button>
                            <button id="nextBtn" onclick="changePage(1)">Fram</button>
                        </div>
                    </div>
                </main>
                
                <footer>
                    <div class="row" id="footer">
                        <div class="col-sm copyright">
                            <!--     <div class="copyright_logos">
                                <a href="https://creativecommons.org/licenses/by/4.0/legalcode">
                                    <img src="assets/img/logos/cc.svg" class="copyright_logo" alt="Creative Commons License"/><img src="assets/img/logos/by.svg" class="copyright_logo" alt="Attribution 4.0 International"/>
                                </a>
                            </div> -->
                            <div class="copyright_text">
                                &#169; 2026 Mariana Suikkanen Gomes, Natalie Isaksson,
                                Jannicke Meijner.
                            </div>
                        </div>
                    </div>
                </footer>
                
                <script>
                    let currentPage = 0;
                    const pages = document.querySelectorAll('.page');
                    const prevBtn = document.getElementById('prevBtn');
                    const nextBtn = document.getElementById('nextBtn');
                    
                    function changePage(direction) {
                    pages[currentPage].classList.remove('active');
                    currentPage += direction;
                    pages[currentPage].classList.add('active');
                    
                    prevBtn.disabled = (currentPage === 0);
                    nextBtn.disabled = (currentPage === pages.length - 1);
                    }
                </script>
            </body>
        </html>
    </xsl:template>
    

    
</xsl:stylesheet>